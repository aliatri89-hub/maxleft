...
export function ShelvesProvider({ children }) {
  ...
  const loadShelves = useCallback(async (userId) => {
    ...
    try {
      ...
      setShelves({ books: [...currentBooks, ...books], currentBooks, movies, shows, games, totalItems: books.length + movies.length + shows.length + games.length });
    } catch (err) {
