...
// ════════════════════════════════════════════════
// BROWSE CARD — VHS tape for New Releases + Streaming
// Tap to flip (sleeve sheet). Headphones sticker = coverage exists.
// Play interaction lives in the sleeve sheet, not on the card.
// ════════════════════════════════════════════════
...
export default function BrowseCard({ data, pushNav, removeNav, onNavigateCommunity }) {
  ...
  const hasCoverage = data.podcast_count > 0;
        ...
        <div onClick={() => openSleeve()} style={{ background: "#1a1612", borderRadius: 5, position: "relative", cursor: "pointer" }}>
          <div style={{ borderRadius: 3, overflow: "hidden", display: "flex", minHeight: 80 }}>
            ...
            <div style={{
              ...
              {/* Headphones sticker — visual hint that coverage exists */}
              {hasCoverage && (
                <div style={{
